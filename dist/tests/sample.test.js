...
var rampUpVUs = new ScenarioConfig("ramp-up-vus").useRampingVUs([
  { duration: "30s", target: 10 },
  { duration: "1m", target: 50 },
  { duration: "30s", target: 0 }
]).setExec("sampleTest");
var options = new K6Config().setThresholds(
  new ThresholdsConfig().setHttpReqDuration(["p(95)<500"]).setHttpReqFailed(["rate<0.01"])
).addScenario(rampUpVUs).build();
var client = new APIClient("GET", "https://test.k6.io");
function sampleTest() {
